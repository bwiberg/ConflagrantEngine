...
void main(void) {
    float snow = CalcSnowNoise(fIn_WorldPosition) * fIn_Snowiness;
    ...
    GNormalShininess.w = (1 - snow) * shininess + snow * SnowShininess;

    GAlbedoSpecular.rgb = (1 - snow) * diffuse.rgb + snow * SnowDiffuse;
    GAlbedoSpecular.a = (1 - snow) * max(max(specular.r, specular.g), specular.b) + snow * SnowSpecular;
}
